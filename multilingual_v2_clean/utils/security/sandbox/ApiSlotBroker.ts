...
export class ApiSlotBroker {
  ...
  public static async acquireSlot(
    ...
    try {
      // 1. Atomik RPC çağrısını gerçekleştir
      const { data, error } = await supabase.rpc('acquire_api_slot', {
        p_job_id: jobId
      });

      if (error) {
        // Hata şema/tablo eksikliğinden kaynaklanıyorsa (PostgREST hatası PGRST202 veya PGRST205)
        if (error.code === 'PGRST202' || error.code === 'PGRST205' || error.message.includes('acquire_api_slot')) {
          ...
          return 'primary'; // Uptime koruması
        }
      ...
      // RPC başarılı olduysa dönen değeri parse et
      if (data === 'primary' || data === 'backup') {
        return data as 'primary' | 'backup';
      }

      return null; // Yuva dolu (tüm kanallar meşgul)
    } catch (err: any) {
      console.error(`❌ [ApiSlotBroker Exception] Beklenmeyen hata: ${err.message}`);
      return 'primary'; // Çökmeyi engellemek için defensive fallback
    }
  }
  ...
  public static async releaseSlot(
    ...
    try {
      ...
      if (error) {
        // RPC bulunamadı hatası dışında kalan kritik hataları logla
