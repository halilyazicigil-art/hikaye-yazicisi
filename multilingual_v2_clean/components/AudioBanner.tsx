...
export default function AudioBanner({ story, source }: AudioBannerProps) {
  ...
  return (
    <div className="mb-6 bg-[#081826] border border-[#182A46] rounded-3xl p-5 sm:p-6 flex flex-col lg:flex-row items-center justify-between gap-6 shadow-2xl animate-in fade-in slide-in-from-top-4 duration-500 overflow-hidden relative">
      ...
      <div className="flex items-center gap-5">
        ...
        <div>
          <h3 className="font-black text-white text-xl leading-tight tracking-tight uppercase">{t('story.audio_banner.title')}</h3>
          <p className="text-[#81808C] text-sm font-bold mt-1">{t('story.audio_banner.desc')}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
        ...
        >
          {isThisPlaying ? t('story.audio_banner.playing') : t('story.audio_banner.play_now')}
        </button>
      </div>
    </div>
  )
