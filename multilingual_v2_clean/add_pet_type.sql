-- 1) Add pet_type column to profile_pets table if it doesn't exist
ALTER TABLE public.profile_pets ADD COLUMN IF NOT EXISTS pet_type TEXT DEFAULT NULL;

-- 2) Re-create complete_story_listen function to handle default pet_type when inserting new rows
...
BEGIN
  -- Check if already logged for this combination
  SELECT EXISTS (
    SELECT 1 FROM public.profile_story_activity
    WHERE profile_id = p_profile_id AND story_id = p_story_id AND source = p_source
  ) INTO v_already_listened;

  IF v_already_listened THEN
    SELECT xp, level, stage INTO v_old_xp, v_old_level, v_old_stage 
    FROM public.profile_pets WHERE profile_id = p_profile_id;

    RETURN jsonb_build_object(
      'success', true,
      'rewarded', false,
      'reward_type', CASE WHEN p_source IN ('library', 'community_popular') THEN 'xp' ELSE 'badge' END,
      'xp_added', 0,
      'pet_stats', jsonb_build_object(
        'xp', COALESCE(v_old_xp, 0),
        'level', COALESCE(v_old_level, 1),
        'stage', COALESCE(v_old_stage, 'egg'),
        'level_up', false,
        'evolved', false
      ),
  ...
  IF p_source IN ('library', 'community_popular') THEN
    ...
    IF NOT FOUND THEN
      ...
      -- Default to 'dragon' type if a pet is automatically initialized via reading completion
