...
export default async function AdminPage({
  ...
  const [
    ...
    supabase.from('users').select('id, email, bonus_quota, is_suspended').order('created_at', { ascending: false }).limit(100),
  ...
  const totalStripeFees = (paymentsData || []).reduce((acc: number, curr: any) => acc + (Number(curr.amount) * 0.029 + 0.3), 0);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 md:p-12">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 mb-12 animate-in fade-in slide-in-from-top-4 duration-700">
          {/* Brüt Gelir */}
        ...
        <div className="bg-white rounded-[2.5rem] shadow-sm border border-orange-100 overflow-hidden mt-12 animate-in fade-in slide-in-from-bottom-10 duration-1000">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-orange-50/20 text-orange-400 text-[10px] font-black uppercase tracking-widest">
                  <th className="px-8 py-5">{t('admin.tables.date')}</th>
                  <th className="px-8 py-5">{t('admin.tables.user')}</th>
                  <th className="px-8 py-5">{t('admin.logs.location')}</th>
                  <th className="px-8 py-5">{t('admin.logs.message')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-orange-50">
                {errorLogs?.map((log: any) => (
                  <tr key={log.id} className="hover:bg-orange-50/10 transition group">
                    <td className="px-8 py-6 text-gray-400 text-xs font-bold">
                      {new Date(log.created_at).toLocaleString(language === 'tr' ? 'tr-TR' : 'en-US')}
                    </td>
                    <td className="px-8 py-6 font-medium text-gray-500 text-sm">
                      {log.user_email || t('admin.logs.anonymous')}
                    </td>
                    <td className="px-8 py-6">
                      <span className="px-3 py-1 bg-gray-100 text-gray-600 rounded-lg text-[10px] font-black tracking-widest border border-gray-200">
                        {log.location}
                      </span>
                    </td>
                    <td className="px-8 py-6 text-red-500 text-sm font-medium">
                      {log.error_message}
                    </td>
                  </tr>
                ))}
