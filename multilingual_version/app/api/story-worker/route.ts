...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        // 🎙️ KARA KUTU LOGLAMA (Hata tespiti için)
        const logError = (msg: string) => {
            try {
                const logPath = path.join(process.cwd(), 'artifacts/scratch/worker_debug.txt');
                fs.appendFileSync(logPath, `[${new Date().toISOString()}] ${msg}\n`);
            } catch (e: any) {}
        };

        logError(`İş Başladı: ${jobId}`);
        ...
        CONTINUITY RULE:
        ${payload.master_ref_story_id ? "Bu bir devam hikayesidir. Önceki hikayedeki karakterleri, onların kişiliklerini ve isimlerini birebir koru. Hikayeyi kaldığı yerden veya o evrenden devam ettir." : ""}
        ...
        const refStoryId = payload.master_ref_story_id || payload.uploaded_master_ref;
        
        if (refStoryId && refStoryId !== 'undefined' && refStoryId !== 'null' && (refStoryId.length > 20 || refStoryId.startsWith('http'))) {
            // Eğer bir story ID ise
            if (!refStoryId.startsWith('http')) {
                const { data: refStory } = await supabase.from('stories').select('image_url, metadata').eq('id', refStoryId).single();
                if (!refStory) throw new Error("Referans hikaye bulunamadı.");
                
                const metadata = (refStory as any).metadata || {};
                // Önce karakter paftasını (master) dene, yoksa ana resmi al
                const targetImageUrl = metadata.master_image_url || refStory.image_url;
                
                if (!targetImageUrl) throw new Error("Referans görseli bulunamadı.");
                ...
                masterUrl = targetImageUrl;
            } else {
                // Eğer doğrudan bir URL ise (Uploaded)
                const imgRes = await fetch(refStoryId);
            ...
            masterMedia = await generateImagePro(masterPrompt, projectId, token as string);
...
CRITICAL INSTRUCTION 2: The image MUST NOT contain any text, letters, words, watermarks, signatures, or typography. Clean visual art only.`;
            ...
            const media = await generateImagePro(scenePrompt, projectId, token as string, [{ name: 'master', data: masterMedia.data }]);
            ...
            for (let i = 0; i < pages.length; i++) {
                ...
                if (audioMedia) {
                    ...
                    if (pagesWithImages[i]) {
                        (pagesWithImages[i] as any).duration = duration;
                    }
                }
        ...
        const { data: story, error: storyErr } = await supabase.from('stories').insert({
            ...
            metadata: {
                voice_name: payload.elevenVoiceId || 'Aoede',
                genre: payload.genre || 'Macera',
                style: payload.style || 'Sulu Boya',
                age_group: payload.age || '4-6',
