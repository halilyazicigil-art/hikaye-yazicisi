...
const Navbar = ({ user }: { user?: any }) => {
  ...
  return (
    <nav className="max-w-7xl mx-auto px-6 py-10 flex justify-between items-center relative z-50">
      ...
      <div className="hidden md:flex items-center gap-2 bg-[#052159]/20 backdrop-blur-3xl px-4 py-2 rounded-full border border-white/30 shadow-[0_10px_40px_rgba(0,0,0,0.2)]">
        {navLinks.map((link) => (
          <Link 
            key={link.name}
            href={link.href} 
            className={`px-5 py-2 rounded-full transition-all text-sm font-black tracking-wide border duration-300 ${
              pathname === link.href 
                ? 'bg-white text-[#052159] border-white shadow-lg scale-105' 
                : 'text-white border-white/20 hover:bg-white/20 hover:border-white/40 hover:scale-105'
            }`}
          >
            {link.name}
          </Link>
